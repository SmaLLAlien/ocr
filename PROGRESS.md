...
**Хвосты для будущих сессий (не блокируют):** живой TTY-прогон `config provider`/`config model`; MCP с реальным сервером; anthropic-протокол живьём; scan dedup-фаза; кросс-совместимость resume с сессиями, записанными Go-бинарником (Go-тулчейна на машине нет).
